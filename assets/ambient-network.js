document.addEventListener('DOMContentLoaded', function() {
  // Create both containers (mobile and desktop)
  const networkContainer = document.createElement('div');
  networkContainer.className = 'ambient-network';
  
  const dotsContainer = document.createElement('div');
  dotsContainer.className = 'ambient-dots-container';
  
  document.body.appendChild(networkContainer);
  document.body.appendChild(dotsContainer);
  
  // Check if mobile - more reliable detection
  ...
  if (isMobile) {
    // Mobile: Create static breathing dots
    createMobileDots();
  } else {
    // Desktop: Create canvas animation
    createDesktopAnimation();
  }
  
  // Mobile dots creation function
  function createMobileDots() {
    ...
    // Create appropriate number of dots based on screen size
    let dotCount;
  ...
  function createDesktopAnimation() {
    ...
    // Configuration
    const config = {
      dotCount: window.innerWidth < 1200 ? 40 : 50,
      dotSize: 2,
      maxDistance: 150,
      mouseRadius: 100,
      speed: 0.3,
  ...
  // Handle window resize - switch between mobile/desktop if needed
  let resizeTimeout;
  window.addEventListener('resize', function() {
    ...
    resizeTimeout = setTimeout(() => {
      ...
      if (newIsMobile !== isMobile) {
        ...
        // Just recreate mobile dots with new count
        createMobileDots();
      }
    }, 250);
  });
